const accParEl = document.querySelector('[data-accordion="accordions"]') as HTMLDivElement;
const accBodyEls = document.querySelectorAll('[data-accordion-body]') as NodeListOf<HTMLDivElement>;
...
accParEl?.addEventListener('click', (event: Event) => {
    ...
    if (target.hasAttribute('data-accordion-icon')) {
        ...
        for (const accBodyEl of accBodyEls) {
            const iconEl = document.querySelector(`[data-accordion-icon='${initIconValue}']`)!;
            const bodyValue = accBodyEl.dataset.accordionBody;
            const bodyEl = document.querySelector(`[data-accordion-body='${bodyValue}']`);
