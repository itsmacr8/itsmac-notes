...
const TIME_VALUE_OF_MONEY_FORM = document.querySelector(
    '[data-time="chapter-four-form"]',
) as HTMLFormElement;
const TIME_VALUE_OF_MONEY_SECTION = document.querySelector(
    '[data-time="time-value-of-money"]',
) as HTMLElement;
...
function changeButtonText(button: HTMLButtonElement) {
    if (button) {
        //  assign the parameter to solve no-param-reassign error
        const btn = button;
